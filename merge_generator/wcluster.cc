...
#define INPUT_DEFAULT_A	1000 /* Part of Generalised Brown */
...
opt_define_int(initA,        "a", INPUT_DEFAULT_A,         "Active set size (alias for C)"); /* Generalised Brown terminology */
...
/**
 * Log file in which to record merge info, part of 
 * Generalised Brown patch for cluster.py compatability.
 */
ofstream merge_log;
...
void compute_L2() {
  ...
  for(int s = 0; s < (int)(slot2cluster).size(); s++) \
  for(bool _tmp = true; slot2cluster[s] != -1 && _tmp; _tmp = false) {
    track_block("L2", "L2[" << Slot(s) << ", *]", false)
#pragma omp parallel for schedule (dynamic,16) 
		for(int t = 0; t < (int)(slot2cluster).size(); t++) \
		    for(bool _tmp = true; slot2cluster[t] != -1 && _tmp; _tmp = false) {
      if(!ORDER_VALID(s, t)) continue;
...
void merge_clusters(int s, int t) {
  ...
  track("Merging clusters", Cluster(b) << " and " << Cluster(a) << " into " << c << ", lost " << L2[s][t], false);


  /**
   * Echo merge info into log file for merges, part of 
   * Generalised Brown patch for cluster.py compatability.
   */
  merge_log << std::setprecision( 10 ) << Phrase( cluster2rep[ b ] ) << "\t" \
	<< Phrase( cluster2rep[ a ] ) << "\t" << ( 2 * ( N - 1 ) - c ) << "\t" \
	<< L2[ s ][ t ] << "\t" << curr_minfo << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  /* Bind value of a to c if user chose that alias instead. C, as 
   * originally implemented, serves the purpose of a in Generalised Brown. 
   */
  if( initA != initC && initA != INPUT_DEFAULT_A ) { initC = initA; }
  ...
  if(!output_dir.empty()) {
    ...
    if(!print_stats) {
      merge_log.open( output_dir + "/merges" ); /* cluster.py compatability. */
    }
  }
